...
### unreliable_chg 是帧级历史取证，不受 X 轴存续过滤影响

`unreliable_chg` 按**该帧原始数据**独立计算，遍历该帧全部合约月，**不经 X 轴
`contracts` 筛**。两者语义不同、不得复用同一列表：

| | 语义 | 依据 |
|---|---|---|
| `contracts` | 展示层决策：X 轴该显示哪些列 | 最后一帧是否仍挂牌，到期即移出 |
| `unreliable_chg` | 帧级历史取证：该帧当时 stored 与 diff 是否不符 | 该帧自身的原始数据 |
...
`derive --test` 的 `2026-07-27` 断言守着这条：把计算改回经 X 轴筛立刻变红
（实测注入 → `FAIL 2026-07-27: 修订合约未被标记 unreliable: {'JUL26'}`）。
