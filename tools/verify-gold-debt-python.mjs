// Native-Windows bridge for the C18C.1 Python guard. The repository root is
...
if (!succeeded(converted) || !converted.stdout.trim()) {
  ...
  const canonical = await run('wsl.exe', ['-d', distro, '--',
    'readlink', '-f', converted.stdout.trim()]);
  if (!succeeded(canonical) || !canonical.stdout.trim()) {
    ...
    const pythonEntry = [
      'import os, runpy, sys',
      'print("C18C1_GUARD_CWD=" + os.path.realpath(os.getcwd()), flush=True)',
      'sys.argv = ["derive_gold_vs_debt.py", "--test"]',
      'runpy.run_path("derive_gold_vs_debt.py", run_name="__main__")',
    ].join('; ');
    const result = await run('wsl.exe', ['-d', distro, '--cd', wslRoot, '--',
      'python3', '-B', '-c', pythonEntry]);
    process.stdout.write(result.stdout);
    process.stderr.write(result.stderr);
    console.log(`C18C1_WINDOWS_ROOT=${root}`);
    console.log(`C18C1_WSL_ROOT=${wslRoot}`);
    const actualCwd = result.stdout.match(/^C18C1_GUARD_CWD=(.+)$/m)?.[1]?.trim();
    const cwdMatches = actualCwd === wslRoot;
    if (!cwdMatches) {
      console.error(`C18C.1 execution path mismatch: cwd=${actualCwd || '<missing>'} expected=${wslRoot}`);
    }
    console.log(`C18C1_PYTHON_EXIT derive_gold_vs_debt.py=${String(result.code)}`);
