  ...
  cases: [
    {
      ...
      expectedFailureMarkers: ['FAIL UST Y 轴按选区内启用 dataset 真实值自动重算'],
    },
    {
      name: 'restricted TradingView yield symbol returns to unavailable contract',
      patch: bytes => replaceExactly(bytes.toString('utf8'),
        '  symbols: Object.freeze([]),',
        "  symbols: Object.freeze(['TVC:US10Y']),"),
      verifyPatch: (_, patched) => ({
        ok: patched.toString('utf8').includes("Object.freeze(['TVC:US10Y'])"),
        detail: 'restricted yield symbol was reintroduced',
      }),
      expectedFailureMarkers: ['FAIL production 不再请求任何受限 TVC / CBOT symbol'],
    },
    {
      name: 'Live market context contaminates fiscal calculations',
      patch: bytes => replaceExactly(bytes.toString('utf8'),
      ...
      verifyPatch: (_, patched) => ({
        ...
        detail: 'unavailable market context now enters fiscal calculations',
      }),
    ...
    },
    {
      ...
      patch: bytes => replaceExactly(bytes.toString('utf8'),
        'Compatibility source: TradingView Advanced Chart Widget',
        'External chart source omitted'),
      verifyPatch: (_, patched) => ({
        ok: !patched.toString('utf8').includes('Compatibility source: TradingView Advanced Chart Widget'),
        detail: 'source attribution sentence removed',
      }),
      expectedFailureMarkers: ['FAIL TradingView compatibility 与 C17 effective_r 隔离说明存在'],
    },
    {
      ...
      expectedFailureMarkers: ['FAIL UST Reset Zoom 按钮与双击恢复结果完全一致'],
    },
  ],
});
